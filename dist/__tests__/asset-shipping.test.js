...
describe('shipped plugin assets — commands/*.md (4 files)', () => {
    ...
    it.each(COMMANDS)('%s exists with valid frontmatter, expected body content, and no mcp__-notation', ({ file, tool, hasArg }) => {
        ...
        if (hasArg) {
            ...
            expect(data['argument-hint'].length).toBeGreaterThan(0);
        }
        else {
...
describe('shipped plugin assets — cross-asset consistency', () => {
    ...
    function extractOutputFormatSection(body) {
    ...
    it('package.json, plugin.json, marketplace.json (metadata.version), README badge, and src/index.ts (McpServer ctor) all match', () => {
        ...
        for (const [where, value] of Object.entries(versionsByLocation)) {
            if (typeof value !== 'string' || value.length === 0) {
                throw new Error(`Version missing or non-string at ${where}. Got: ${JSON.stringify(value)}\n` +
