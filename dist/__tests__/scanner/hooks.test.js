...
describe('scanHooks', () => {
    ...
    describe('Minor 1 — User-Hooks aus ~/.claude.json', () => {
        ...
        it('does not crash when ~/.claude.json is missing or malformed', async () => {
            await expect(scanHooks(tmpDir, undefined, path.join(tmpDir, 'nonexistent.json'))).resolves.not.toThrow();
    ...
    describe('Minor 1 — Plugin-Hooks', () => {
        ...
        it('scans plugin hooks from <plugin-root>/hooks/hooks.json', async () => {
            ...
            fs.writeFileSync(path.join(sp, 'hooks', 'hooks.json'), JSON.stringify({
                hooks: {
                    SessionStart: [
                        {
                            matcher: '',
            ...
            fs.writeFileSync(path.join(cm, 'hooks', 'hooks.json'), JSON.stringify({
                hooks: {
                    PreCompact: [
                        {
                            matcher: '',
        ...
        it('marks plugin hooks with plugin/marketplace/version metadata', async () => {
            ...
            fs.writeFileSync(path.join(plug, 'hooks', 'hooks.json'), JSON.stringify({
                hooks: {
                    PostToolUse: [
        ...
        it('returns no plugin hooks when pluginsPath is missing', async () => {
            const entries = await scanHooks(tmpDir, undefined, undefined, path.join(tmpDir, 'missing-plugins'));
            const pluginHooks = entries.filter((e) => e.metadata.hookScope === 'plugin');
            expect(pluginHooks).toEqual([]);
        });
    });
