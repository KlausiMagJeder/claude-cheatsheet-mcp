...
describe('scanSkills', () => {
    ...
    it('Layout A: scans skills under cache/<marketplace>/<plugin>/<version>/skills/', async () => {
        const pluginRoot = path.join(tmpDir, 'cache', 'claude-plugins-official', 'superpowers', '5.0.7');
    ...
    it('Layout B: follows skills-array from plugin.json pointing to .claude/skills/', async () => {
        ...
        writeSkill(path.join(pluginRoot, '.claude', 'skills', 'ui-ux-pro-max'), 'ui-ux-pro-max', 'UI/UX helper');
        const entries = await scanSkills(tmpDir);
        const names = entries.map((e) => e.name);
    ...
    it('Layout C: skips plugins without skills folder (commands-only)', async () => {
        ...
        fs.writeFileSync(path.join(pluginRoot, 'commands', 'setup.md'), '---\nname: setup\n---\nSetup.');
        const entries = await scanSkills(tmpDir);
    ...
    it("accepts version directory named 'unknown'", async () => {
        const pluginRoot = path.join(tmpDir, 'cache', 'claude-plugins-official', 'pr-review-toolkit', 'unknown');
    ...
    it('detects deprecated skills via description', async () => {
        ...
        writeSkill(path.join(pluginRoot, 'skills', 'old-skill'), 'old-skill', 'Deprecated - use new-skill instead');
        const entries = await scanSkills(tmpDir);
    ...
    it('is resilient against symlink cycles', async () => {
        ...
        try {
            fs.symlinkSync(path.join(pluginRoot, 'skills'), path.join(pluginRoot, 'skills', 'loop'), 'dir');
        }
        catch {
    ...
    describe('Minor 2 — repos/-Fallback', () => {
        it('scans skills from ~/.claude/plugins/repos/<plugin>/skills/<name>/SKILL.md', async () => {
            ...
            writeSkill(path.join(repoPlugin, 'skills', 'dev-skill'), 'dev-skill', 'Locally developed skill');
            const entries = await scanSkills(tmpDir);
            const names = entries.map((e) => e.name);
    ...
    it('prefers installed_plugins.json when present', async () => {
        ...
        fs.writeFileSync(path.join(tmpDir, 'installed_plugins.json'), JSON.stringify({
