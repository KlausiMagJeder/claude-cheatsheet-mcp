...
describe('scanCommands', () => {
    ...
    beforeEach(() => {
        ...
        fs.writeFileSync(path.join(globalCmds, 'jarvis.md'), `---
name: jarvis
...
Jarvis content here.`);
        ...
        fs.writeFileSync(path.join(projectCmds, 'fix-rubocop.md'), `---
...
Rubocop fixer content.`);
    ...
    describe('Blocker 2 — Plugin-Commands', () => {
        ...
        function writeCommand(dir, name, description) {
        ...
        it('scans commands from <plugin-root>/commands/*.md (Layout A)', async () => {
            ...
            const superpowers = makePluginRoot(pluginsRoot, 'claude-plugins-official', 'superpowers', '5.0.7');
        ...
        it('handles multiple plugins in parallel', async () => {
            ...
            const entries = await scanCommands(tmpDir, undefined, pluginsRoot);
            const pluginCmds = entries.filter((e) => e.metadata.plugin === 'plugin-a' || e.metadata.plugin === 'plugin-b');
