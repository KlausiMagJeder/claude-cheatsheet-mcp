 ...
 */
export async function scanSkills(pluginsPath) {
 ...
 * Plugins) gescannt.
 */
async function discoverPluginContexts(pluginsPath) {
    const installed = await readInstalledPlugins(pluginsPath);
    const cacheContexts = installed !== null
 ...
 */
async function discoverRepoContexts(pluginsPath) {
 ...
 */
async function discoverPluginContextsViaGlob(pluginsPath) {
 ...
 */
function deriveContextFromPath(pluginRoot, cacheDir) {
 ...
 */
async function findSkillFilesForPlugin(ctx) {
 ...
 */
async function globSkillMd(searchDir) {
    let exists = false;
    try {
 ...
 */
async function readDeclaredSkillPaths(pluginRoot) {
    ...
    if (!Array.isArray(skills))
        return null;
    return skills.filter((item) => typeof item === 'string');
}
/**
 * Parst eine einzelne `SKILL.md` in einen `CatalogEntry`.
 */
async function parseSkillFile(filePath, ctx) {
    let stat;
    try {
    ...
    const deprecatedInText = description.toLowerCase().includes('deprecated') ||
    ...
    if (redirectsTo !== undefined)
        metadata.redirectsTo = redirectsTo;
    return {
