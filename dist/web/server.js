...
export function startWebServer(state, staticDir, port = 37778) {
    ...
    server.on('error', (err) => {
        if (err.code === 'EADDRINUSE') {
            console.error(`[web-server] port ${port} already in use — dashboard not started. ` +
...
async function handleRequest(req, res, router, dashboardPath) {
