...
const USER_CONFIG_PATH = process.env.CLAUDE_USER_CONFIG_PATH ?? path.join(os.homedir(), '.claude.json');
...
server.registerTool('list_mcp_tools', {
...
server.registerTool('list_cli_commands', {
...
server.registerTool('get_detail', {
...
// Bedarfsanalyse A4 verlangt Graceful Shutdown. Ohne Handler beendet Node
...
async function main() {
    ...
    state.index = await buildIndex(GLOBAL_PATH, PROJECT_PATH, STATIC_DIR, USER_CONFIG_PATH, PLUGINS_PATH);
    // Kill-Switch (Bedarfsanalyse O8): In Umgebungen, in denen das Dashboard
