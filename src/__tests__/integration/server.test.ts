...
function frame(req: JsonRpcRequest): string {
 ...
 */
function waitForJsonLine(proc: ChildProcess, timeoutMs: number): Promise<JsonRpcResponse> {
 ...
 */
function collectStdoutLines(proc: ChildProcess, durationMs: number): Promise<string[]> {
...
beforeAll(() => {
  if (!fs.existsSync(DIST_ENTRY)) {
    throw new Error(
      `dist/index.js not found at ${DIST_ENTRY}. ` +
