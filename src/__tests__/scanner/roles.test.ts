...
describe('scanRoles', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(
      path.join(roleDir, 'senior-rails-developer.md'),
...
Verantwortlich fuer die Implementierung von Rails-Features.`,
    ...
    fs.writeFileSync(
      path.join(role2Dir, 'test-engineer.md'),
      `# Test Engineer
...
Verantwortlich fuer Tests und Qualitaetssicherung.`,
    ...
    fs.writeFileSync(
      path.join(jarvisDir, 'faehigkeiten-pro-rolle.md'),
      `# Faehigkeiten pro Rolle\n\nMetadata file.`,
    );
    fs.writeFileSync(
      path.join(jarvisDir, 'rollenverwaltung.md'),
      `# Rollenverwaltung\n\nAdmin file.`,
    );
  });
