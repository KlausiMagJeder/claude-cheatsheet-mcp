...
describe('plugin manifests — .claude-plugin/plugin.json', () => {
  ...
  beforeAll(() => {
    ...
    plugin = readJson(PLUGIN_JSON_PATH) as Record<string, unknown>;
  });
  ...
  it('has a non-empty license string (empirical 5/7 plugins set one)', () => {
    ...
    expect((plugin.license as string).length).toBeGreaterThan(0);
  });
});
...
describe('plugin manifests — .mcp.json', () => {
  ...
  beforeAll(() => {
    ...
    mcp = readJson(MCP_JSON_PATH) as { mcpServers?: Record<string, unknown> };
  });
...
describe('plugin manifests — .claude-plugin/marketplace.json', () => {
  ...
  beforeAll(() => {
    ...
    marketplace = readJson(MARKETPLACE_JSON_PATH) as Record<string, unknown>;
  });
  ...
  it('has required field: name (non-empty string)', () => {
    ...
    expect((marketplace.name as string).length).toBeGreaterThan(0);
  });
...
describe('plugin manifests — cross-manifest consistency', () => {
  it('plugin.json name equals the marketplace plugin entry name', () => {
    const plugin = readJson(PLUGIN_JSON_PATH) as Record<string, unknown>;
    const marketplace = readJson(MARKETPLACE_JSON_PATH) as Record<string, unknown>;
    const plugins = marketplace.plugins as Array<Record<string, unknown>>;
...
describe('plugin manifests — build output referenced from .mcp.json', () => {
  ...
  it('dist/index.js exists after build (warn-only if missing)', () => {
    if (!existsSync(DIST_INDEX_PATH)) {
      console.warn(
        `[plugin-manifests] dist/index.js not found at ${DIST_INDEX_PATH} — run "npm run build" before activating the plugin. Skipping hard assertion.`,
      );
      return;
    }
