...
function makeEntry(overrides: Partial<CatalogEntry> & { kind: EntryKind }): CatalogEntry {
...
function makeIndex(entries: CatalogEntry[]): CatalogIndex {
  return {
    entries,
...
describe('getPluginDetail — happy path', () => {
  ...
  it('returns short entries (no filePath, no metadata leak)', () => {
    ...
    expect((short as unknown as Record<string, unknown>).filePath).toBeUndefined();
    expect((short as unknown as Record<string, unknown>).metadata).toBeUndefined();
  });
});
...
describe('getPluginDetail — plugin-scoped kinds only', () => {
  it('excludes role and cli_command entries even if synthetically tagged with metadata.plugin', () => {
    ...
    const entriesRecord = result.entries as unknown as Record<string, unknown>;
    ...
    const totalsRecord = result.totals as unknown as Record<string, unknown>;
