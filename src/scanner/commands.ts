...
async function scanCommandDir(dir: string, scope: 'global' | 'project'): Promise<CatalogEntry[]> {
  const entries: CatalogEntry[] = [];
  ...
  for (const filePath of files) {
    ...
    if (entry) {
      if (scope === 'project') entry.metadata['projectPath'] = dir;
      entries.push(entry);
    }
  }
...
async function scanPluginCommands(pluginsPath: string): Promise<CatalogEntry[]> {
 ...
 */
async function findPluginCommandFiles(ctx: PluginCtx): Promise<string[]> {
...
async function readDeclaredCommandPaths(pluginRoot: string): Promise<string[] | null> {
