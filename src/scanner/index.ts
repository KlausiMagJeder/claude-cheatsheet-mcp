...
import type { TagOverride } from '../tagger/index.js';
...
export async function buildIndex(
  ...
  userConfigPath: string = path.join(os.homedir(), '.claude.json'),
  pluginsPath: string = path.join(globalPath, 'plugins'),
  ...
  const entries: CatalogEntry[] = results.flatMap((result) =>
    result.status === 'fulfilled' ? result.value : [],
  );

  // Load tag overrides (optional).
  let overrides: Record<string, TagOverride> = {};
  try {
    const overridesPath = path.join(staticDir, 'tag-overrides.json');
    const content = await fs.readFile(overridesPath, 'utf-8');
    overrides = JSON.parse(content) as Record<string, TagOverride>;
  } catch {
